...
export default function SimpleARModal({ 
  open, 
  onClose, 
  gantt, 
  glbUrl, 
  ...
  useEffect(() => {
    ...
    const checkARCapabilities = async () => {
      ...
      try {
        ...
        const [hasPermission, isMobile, isSecure] = await Promise.all([
        ...
        if (!isSecure) {
          ...
          setArState('unsupported');
          setError('AR is optimized for mobile devices');
        } else {
          setArState('ready');
          setError(null);
        }
  ...
  const startARSession = async () => {
    ...
    try {
      ...
      // Configure AR modes for mobile
      const arModes = mobileDevice ? 'scene-viewer quick-look webxr' : 'webxr';
    ...
    <div className="flex flex-col items-center justify-center h-96 bg-gradient-to-br from-blue-50 to-purple-50 rounded-xl">
      <div className="text-center space-y-4">
        ...
        <p className="text-sm text-slate-600 max-w-xs">
          {error || 'Your device doesn\'t support AR features. Try using a mobile device with AR capabilities.'}
        </p>
        <Button 
          ...
          <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-700">
            <div className="grid grid-cols-2 gap-4 text-xs text-slate-500">
              <div className="flex items-center gap-2">
                ...
                <span>WebXR: {mobileDevice ? 'Available' : 'Not Available'}</span>
              </div>
              <div className="flex items-center gap-2">
