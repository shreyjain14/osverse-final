...
export default function ARViewerPage() {
  ...
  const [arState, setArState] = useState<'loading' | 'ready' | 'error' | 'unsupported' | 'launching'>('loading');
  ...
  const [autoLaunchAttempted, setAutoLaunchAttempted] = useState<boolean>(false);
      ...
      exporter.parse(
        ...
        (gltf: ArrayBuffer | { [key: string]: unknown }) => {
          if (gltf instanceof ArrayBuffer && gltf.byteLength > 0) {
            ...
            // Auto-attempt AR launch on mobile if not already attempted
            if (!autoLaunchAttempted && isMobileDevice()) {
              setAutoLaunchAttempted(true);
              setTimeout(() => {
                setArState('launching');
                attemptDirectARLaunch();
              }, 1000);
            }
          } else {
  ...
  // Attempt to launch AR directly
  const attemptDirectARLaunch = () => {
    if (!glbUrl) return;

    try {
      // Try to launch Scene Viewer directly for Android
      if (/Android/i.test(navigator.userAgent)) {
        const sceneViewerUrl = `intent://arvr.google.com/scene-viewer/1.0?file=${encodeURIComponent(glbUrl)}&mode=ar_only#Intent;scheme=https;package=com.google.ar.core;action=android.intent.action.VIEW;S.browser_fallback_url=${encodeURIComponent(window.location.href)};end;`;
        window.location.href = sceneViewerUrl;
      } 
      // Try to launch AR Quick Look for iOS
      else if (/iPhone|iPad|iPod/i.test(navigator.userAgent)) {
        // iOS requires .usdz format for AR Quick Look, so fall back to model-viewer
        setArState('ready');
        startARSession();
      } else {
        setArState('ready');
      ...
      console.error('Failed to launch direct AR:', error);
      setArState('ready');
    }
  };
  ...
  // AR launching view
  if (arState === 'launching') {
    return (
      <div className="min-h-screen bg-gradient-to-br from-green-50 to-blue-50 flex items-center justify-center p-4">
        <div className="text-center space-y-4">
          <div className="animate-pulse rounded-full h-16 w-16 bg-green-200 flex items-center justify-center mx-auto">
            <Camera className="w-8 h-8 text-green-600" />
          </div>
          <h1 className="text-xl font-semibold text-slate-900">Launching AR...</h1>
          <p className="text-slate-600">Opening AR viewer on your device</p>
          <div className="mt-4">
            <Button 
              onClick={() => setArState('ready')}
              variant="outline"
      ...
      <div className="p-4">
        <div className="max-w-md mx-auto space-y-4">
          ...
          <div className="space-y-3">
            ...
            <Button 
              onClick={() => glbUrl && window.open(glbUrl, '_blank')}
              variant="outline"
